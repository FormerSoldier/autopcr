...
class labyrinth_start_reroll(Module):
    ...
    def _validate_cost_effective(self, area: int, route: List, difficulty: int) -> Tuple[bool, str]:
        ...
        if difficulty not in (4, 5):
            return True, ""
        if area == 4:
            hard_count = sum(1 for b in route if self._block_type(b) == eLabyrinthBlockType.HARD_QUEST)
            if hard_count < 2:
                return False, f"区域4仅{hard_count}个hard(EX)，需双hard"
        if area == 2:
            types = [self._block_type(b) for b in route]
            ok = any(
                types[i] == eLabyrinthBlockType.RELIC and i + 1 < len(types) and types[i + 1] == eLabyrinthBlockType.HARD_QUEST
    ...
    def _find_area_route(self, area: int, map_list: List, area3_bosses: Set[int], area5_bosses: Set[int], perfect_start: bool, expected_block_types: Dict[int, Dict[int, Set[eLabyrinthBlockType]]], start_mode: str, difficulty: int) -> Tuple[Optional[List], str]:
    ...
    def _find_routes(self, map_list: List, difficulty: int, area3_bosses: Set[int], area5_bosses: Set[int], perfect_start: bool, expected_block_types: Dict[int, Dict[int, Set[eLabyrinthBlockType]]], start_mode: str) -> Tuple[Optional[Dict[int, List]], str]:
        routes: Dict[int, List] = {}
        failures = []
        for area in self._target_areas(difficulty):
            route, reason = self._find_area_route(area, map_list, area3_bosses, area5_bosses, perfect_start, expected_block_types, start_mode, difficulty)
            if not route:
    ...
    async def do_task(self, client: pcrclient):
        ...
        for attempt in range(1, max_count + 1):
            ...
            routes, reason = self._find_routes(enter.map_list or [], difficulty, area3_bosses, area5_bosses, perfect_start, expected_block_types, start_mode)
            if routes:
